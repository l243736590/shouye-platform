...
const partnerShowcases = [
  ...
  },
  {
    ...
    description: '适合承接留学生打工、毕业后求职和短期项目需求，可接入企业认证、岗位审核和投递入口。',
    tags: ['兼职', '招聘', '本地岗位'],
    tone: 'orange',
  },
  {
    ...
    deepDives: [
      {
        ...
        sources: [
          { label: 'Study in Korea 官方留学入口', url: 'https://www.studyinkorea.go.kr/en_US/main.do' },
          { label: '韩国签证门户 Korea Visa Portal', url: 'https://www.visa.go.kr' },
        ],
      },
      {
        ...
        sources: [
          { label: 'Study in Korea 签证与滞留信息', url: 'https://www.studyinkorea.go.kr/en_US/plan/visaAndStay.do' },
          { label: 'HiKorea 电子民愿与预约', url: 'https://www.hikorea.go.kr' },
        ],
      },
    ],
  },
    ...
    deepDives: [
      {
        ...
        sources: [
          { label: 'Study in Korea 留学生就业/打工信息', url: 'https://www.studyinkorea.go.kr/en_US/life/job.do' },
          { label: 'HiKorea 时间制就业许可入口', url: 'https://www.hikorea.go.kr' },
        ],
      },
      {
        ...
        sources: [
          { label: '首尔住房综合门户', url: 'https://housing.seoul.go.kr' },
          { label: '韩国国民健康保险公团英文入口', url: 'https://www.nhis.or.kr/english/' },
        ],
      },
    ],
  },
  {
    ...
    deepDives: [
      ...
      },
      {
        ...
        sources: [
          { label: 'HiKorea 滞留延期与电子民愿', url: 'https://www.hikorea.go.kr' },
          { label: 'Study in Korea 签证与滞留信息', url: 'https://www.studyinkorea.go.kr/en_US/plan/visaAndStay.do' },
        ],
      },
    ],
    ...
    deepDives: [
      {
        ...
        sources: [
          { label: 'HiKorea 滞留资格变更/延期入口', url: 'https://www.hikorea.go.kr' },
          { label: 'Korea Visa Portal 签证类型查询', url: 'https://www.visa.go.kr' },
        ],
      },
      {
        title: '韩国永驻/长期居留路径',
    ...
    <main className={mainClassName}>
        ...
        <section className="info-page">
          ...
          <div className="reward-rule-list">
            <article>
              <span>1</span>
              <h3>回答悬赏问题</h3>
              <p>回答被提问者采纳后获得悬赏积分。积分用于站内权益、作者等级和合作激励，当前不做真实现金提现。</p>
            </article>
            <article>
              <span>2</span>
              <h3>发布高质量经验帖</h3>
              <p>内容被收藏、点赞、加精后可获得平台奖励积分，用于创作者等级、内容曝光和合作权益。</p>
            </article>
            <article>
              <span>3</span>
              <h3>贡献专题攻略</h3>
            ...
            <article>
              <span>4</span>
              <h3>防止垃圾内容</h3>
              <p>抄袭、批量生成的无效内容和答非所问不会获得奖励，严重时扣分、禁言或封号。</p>
            </article>
          </div>
        </section>
      )}
        ...
        <section className="info-page journey-topic-page">
          ...
          {selectedJourneyTopic.deepDives?.length ? (
            <section className="journey-topic-section journey-deep-section">
              <div className="section-heading">
                <p className="eyebrow dark">重点干货与资料样本</p>
                <h2>把热门问题拆成材料、流程、风险和参考入口。</h2>
              </div>
                    ...
                    {activeDeepDiveTitle === item.title && (
                      <div className="journey-deep-content">
                        ...
                        <div className="journey-deep-links">
                          {(item.sources ?? (item.sourceLabel && item.sourceUrl ? [{ label: item.sourceLabel, url: item.sourceUrl }] : [])).map(
                            (source, sourceIndex) => (
                              <a
                                className={sourceIndex === 0 ? undefined : 'source-link'}
                                href={source.url}
                                key={`${item.title}-${source.label}`}
                                target="_blank"
                                rel="noreferrer"
                              >
                                {sourceIndex === 0 ? '官方入口' : '参考'}：{source.label}
                              </a>
                            ),
                          )}
                        </div>
              ...
              {selectedJourneyGuides.length ? (
                ...
                selectedJourneyTopic.categories.slice(0, 4).map((category) => (
                  <article key={category}>
                    <span>{category}</span>
                    <h3>{category}问题合集</h3>
                    <p>围绕这个阶段持续沉淀高频问题、学校差异和真实材料清单。</p>
                    <small>持续更新</small>
                  </article>
                ))
              )}
      ...
      <section className="partner-showcase-section" id="schools">
        <div className="section-heading partner-showcase-heading">
          ...
          <p>
            展示招聘入口、外卖广告、中餐商家和校区服务等合作形态，平台可承接商家资料、评价展示和后台审核。
          </p>
        </div>
